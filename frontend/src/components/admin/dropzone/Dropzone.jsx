...
const Dropzone = ({ images, setImages }) => {
  ...
  useEffect(() => {
    ...
    console.log(images)
    return (
        <section className="mt-16 exo">

<div className="flex items-center justify-">
                <button
                    onClick={() => myCropWidget.open()}
                    className="mt-1 text-[17px] uppercase tracking-wider font-bold text-neutral-500 border border-slate-600 border-primary rounded-sm py-2 px-4 hover:bg-primary hover:text-white transition-colors"
                >
                    Upload Image
                </button>
            </div>
        ...
        images.map((item, index) => {
          ...
          return(
            <div key={index} className="relative w-fit">
              <div className="absolute w-6 rounded-full cursor-pointer hover:animate-spin flex items-center justify-center h-6 bg-white -top-2 border -right-2" onClick={() => removeItemAtIndex(index)}>
              <ImCancelCircle className="text-xl text-red-500 cursor-pointer"/>
              </div>
              {/* <img src={item} className=" w-[100%] h-auto rounded-md"/> */}
              <Suspense fallback={<div>Loading...</div>}>
            <LazyLoadedImage src={item} className=" w-[100%] max-h-[10rem] h-auto rounded-md" />
          </Suspense>
            </div>
          )
        })
