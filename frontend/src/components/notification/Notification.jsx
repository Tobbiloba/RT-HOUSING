...
//   console.log(notifications)
  ...
  useEffect(() => {
    ...
    if(notifications.length > 0 && !loading && notifications) {
...
//   }, [])
  ...
  return (
    <div className='fixed w-[90vw] md:w-[30rem] border right-6 top-[13rem] md:top-[5rem] h-fit bg-slate-800'>
      <h1 className='border-b p-4 text-[14px] font-[600]'>Notifications</h1>
      <div className='border flex-1 h-[25rem] '>
        {
            loading ? <div className='flex justify-center py-[4rem]'>
                <CircularProgress />

            </div> : notifications.length > 0 ? <div className='overflow-y-scroll h-[100%]'>
            {
                 notifications.map((item, index) => <NotificationCard item={item} key={index}/>)
             } 
         </div> : <p className='text-center py-4 text-[13px]'>You do not have any notifications.</p>
        }
