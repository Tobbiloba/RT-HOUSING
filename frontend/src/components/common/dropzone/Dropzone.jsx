...
const LazyLoadedImage = lazy(() => import('../lazy loading/LazyLoadedImage'))

const Dropzone = ({ field, form, error, multiple, red }) => {
  ...
  return (
    <section className="mt-1 exo">
      ...
      {error && <div className={`${red ? "text-red-500" : "text-white"} text-[13px] mt-1`}>{error}</div>}
