...
const BreadCrumb = () => {
  ...
  return (
    ...
    >
      <div className="text-slate-300 hidden md:flex flex-col">
...
{pathname.map((item, index) => (
  index === 0 ? (
    // Render the first item differently (for example, without the leading '/')
    <h1 key={index}>{item}</h1>
  ) : index < 4 ? (
    // Render the next two items with a leading '/'
    <h1 className='text-slate-300' key={index}> / {item}</h1>
  ) : null
))}
      </div>
    </div>
