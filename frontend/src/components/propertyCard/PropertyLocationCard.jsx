...
const PropertyLoadingCard = () => {
  ...
  return (
    <div className='h-[17rem] text-white p-[1rem] pb-[2rem] flex flex-col justify-between border relative bg-slate-400 animate-pulse'>
    </div>
  )
}
// @ts-ignore
...
{
  loading ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-12 mt-4">
      {[1, 2, 3, 4].map((item) => (
  ...
  .map(item => (
    <div
      key={item.id}
      style={{ backgroundImage: `url(${item.property_information.property_images[0]})` }}
    ...
    >
      ...
      <div className="z-10 relative flex flex-row items-center justify-between">
        ...
        <p className="text-[13px]">{item.property_information.property_images.length} Photos</p>
      </div>
      ...
      <div className="flex flex-row justify-between items-end relative z-10">
        <div className="text-white ">
          ...
          <p className="text-[13px]">{item.property_information.property_no_bedrooms} Bedroom apartment</p>
          <p className="text-[12px]">
            {item.property_information.property_location.state}, {item.property_information.property_location.city}
          </p>
        </div>
