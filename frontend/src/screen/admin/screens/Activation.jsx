...
import { activateAdminUser, activateUser } from "@/action/auth";
...
const AdminActivation = () => {
  ...
  return (
    <div className="px-[1rem] min-h-[40rem] flex justify-center items-center">
      ...
      <h1 className="text-slate-500">Please wait ...</h1>
      </div> : !loading && !error ? <div className="h-[100%] exo border max-w-[50rem] w-[100%] pb-[3rem] border-slate-400 bg-green-100  flex flex-col items-center">
          <div
          ...
          >
            ...
            <strong className="font-bold text-xl mt-6">Account Activated!</strong>
            ...
            <p className="text-sm mt-2">
              Start exploring and enjoy the convenience of renting with Fabtos!
            </p>
          </div>
        </div>  : !loading && error && <div className="h-[100%] exo max-w-[50rem] w-[100%] pb-[3rem] flex flex-col items-center">
          <img
          ...
          >
            ...
            <p className="text-sm mt-2">
              Oops! Something went wrong during the account activation process.
            </p>
            <p className="text-sm mt-2">
       ...
       {
        !loading && !error &&  <button className="mt-12 border text-green-500 px-6 py-2 rounded-md shadow-md">Please wait redirecting to home in {countdown}</button>
       }
            
        

       {
        !loading && error &&  <button className="mt-12 border bg-red-600 text-white px-6 py-2 rounded-md shadow-md">Go back home</button>
       }
      </div>
    </div>
  );
